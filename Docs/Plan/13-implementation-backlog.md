...
The exception applies only to dependency-safe implementation through LM-080. H9 then runs
once; LM-081 and every later story require their nonremoved dependencies to be `passed`.
...
| LM-040 | Redesign the shared Search surface around one prominent `Search Memory` / `Ask Agent` composer plus visible time pills, approved-site tokens, recognizable application tiles, filter removal, and metadata autocomplete. Reuse the root-owned `SearchSessionModel`; Ask Agent must use detected targets and bounded CLI/MCP policies. | LM-036, LM-039; ADR 0008 | Search and agent route state, every filter, focus target, and unavailable/permission state has deterministic model/accessibility evidence and default/minimum snapshots. Real keyboard/VoiceOver, client detection, and routing remain H9-only. |
| LM-041 | Redesign the adaptive result surface as a polished visual-memory grid with pagination, selection, thumbnail cache, quiet provenance, and initial/loading/adding-visual/no-result/error states. | LM-012, LM-033, LM-039, LM-040 redesign readiness | 10k-card model correctness, stable selection/reuse, action wiring, accessibility announcements, and safe snapshots pass; no stale thumbnail can appear. Production performance and interaction remain H9-only. |
| LM-042 | Move AX/OCR/app/title/URL provenance and diagnostics into honest secondary disclosure while preserving no-summary behavior and exact source labels. | LM-041 redesign readiness | Each golden result cites only validated evidence; disclosure has deterministic keyboard/accessibility models and snapshots. Production UI/accessibility inspection remains H9-only. |
| LM-043 | Redesign detail as a dominant foreground-window screenshot canvas with compact chrome, flanking previous/next controls, lower-leading date/time, secondary provenance/actions, exact-source software-HEIC decode/cache, zoom/pan, corrupt recovery, and export. | LM-025, LM-041 redesign readiness; ADR 0008 | Safe canvas state/action/accessibility models and populated/loading/corrupt/empty snapshots pass. Real codec latency, application interaction, tamper/corruption, and export evidence remain H9-only. |
...
| LM-045 | Redesign the persistent bottom timeline rail with app-colored intervals, real/fallback app symbols, typed patterned gaps, draggable playhead, four working zoom levels, immediate thumbnail selection, dwell promotion, and accessibility-list projection. | LM-043 redesign readiness, LM-044 | Safe scrub/selection/zoom/gap models, keyboard/accessibility models, and appearance/geometry snapshots pass. Real S6 timing, dragging, stale-pixel, keyboard/VoiceOver, resize, and appearance behavior remain H9-only. |
| LM-046 | Make Timeline the primary visual-memory destination with compact Search/Settings actions, canvas previous/next, spatial selection, date navigation, app transitions, selected-search context, and fail-closed Revisit. Remove obsolete parallel three-pane UI. | LM-045 redesign readiness | Safe day/restoration/DST/action/accessibility models and populated/empty/gap/loading snapshots pass; no control is decorative. Real window navigation, NSWorkspace, keyboard/VoiceOver, and drag behavior remain H9-only. |
| LM-047 | Integrate Forget Moment/Range into secondary disclosure while preserving provisional suppression, verified rewrite, progress/failure, and Cancel-default confirmation. | LM-026, LM-043 redesign readiness, LM-045 redesign readiness | Safe action, confirmation, accessibility, suppression, and recovery models plus snapshots pass; hidden targets never reappear. Real installed flow and physical rewrite remain H9-only. |
| LM-048 | Converge the redesigned Recall product and Settings IA. Implement General, Agents, Appearance, Capture, Storage, and Exclusions with real existing services; persist Light/System/Dark selection with fresh-profile Light default; produce the complete deterministic matrix and independent side-by-side design review. | LM-040–LM-047 redesign readiness; LM-057, LM-061, LM-070, LM-080 implementation-ready | Every major surface passes focused unit, snapshot, keyboard-model, accessibility-model, formatting, privacy/static, and native arm64 compile evidence; all visible controls are wired or absent; intentional reference differences are documented. Installed journeys, real appearance/focus/VoiceOver, client detection, media, and UI timing remain H9-only. |
...
| LM-064 | Complete every safe trust/lifecycle implementation and fixture for exclusions, private browsing, pause, retention, encryption copy, delete moment/range/all, export, key loss, and permission revocation; register the unchanged runtime-only trust journey in H9. | LM-056–LM-063 implementation-ready | Safe unit/model/static/compile evidence is complete and the story is explicitly implementation-ready but technically blocked; H9 later supplies every original runtime item, promotes source stories in dependency order, verifies UI claims, and attaches the five-minute trust checklist plus `Results/H9/isolated-validation-ledger.json`. |
...
| ID | Deliverable | Depends on | Acceptance and evidence |
|---|---|---|---|
| LM-080 | Add release performance telemetry that remains local: signposts, bounded rotating logs, diagnostic export, energy/memory/storage/index backlog dashboards. Its existing safe checkpoint remains valid; H9 becomes the next prerequisite only after the ADR-0008 LM-048 redesign checkpoint also passes. | LM-074 | No content in metrics/logs; safe overhead/model and diagnostic-bundle privacy snapshots pass. The original live overhead/dashboard criteria join H9; `Results/H9/isolated-validation-ledger.json` must pass and promote every deferred story before LM-081 begins. |
| LM-081 | Complete three real workdays of personal dogfood, logging search failures, false exclusions, resource discomfort, crashes, and UX friction; fix release blockers. Before observation begins, Codex must verify H9 passed, checkpoint the promoted H9 state, enter the H5 pause, end its active turn, and remain fully stopped until the user manually returns. | LM-080; H9; H5 | Three-workday report, resource percentiles, issue dispositions, zero known privacy/deletion blocker, and evidence that no Codex automation, heartbeat, polling, or monitoring ran during observation. |
