...
| LM-043 | Implement detail canvas and actor-owned exact-source-HEIC decode/cache through the pinned software codec with zoom/pan, frame stepping, corrupt-media recovery, and export-moment action. | LM-025, LM-041 | Decode latency gates pass under the encoder-service tripwire; rapid selection cancellation shows the exact requested frame; runtime tamper, corrupt file, or manifest mismatch yields recovery UI. Implementation is ready from unit/model/static/compile/snapshot proof, but LM-043 remains technically blocked: the unchanged real codec-latency and application-runtime recovery/interaction/export ledger is deferred to H9 under ADR 0006, and safe proof is not a substitute. |
...
| LM-045 | Implement 96-point timeline rail, low-res immediate scrub/full-frame dwell, gap patterns, zoom levels, and accessibility-list projection. | LM-043, LM-044 | S6 scrub budget, keyboard transitions, VoiceOver list, and screenshot baselines pass. Implementation is ready from unit/model/static/compile/snapshot proof, but LM-045 remains technically blocked: the unchanged real S6 timing, stale-pixel, keyboard/VoiceOver, resize/theme/pseudo-localization, and screenshot-baseline ledger is deferred to H9 under ADR 0006, and safe proof is not a substitute. |
| LM-046 | Implement Timeline section, date navigation, app transition labels, selected-result context, and safe Revisit to app/approved URL only. | LM-045 | Day navigation/restoration and locale/DST tests pass; Revisit never restores form state or bypasses policy. Implementation is ready from unit/model/static/compile/snapshot proof, but LM-046 remains technically blocked: real application date/navigation restoration, keyboard/VoiceOver presentation, transition labeling, and NSWorkspace Revisit execution are deferred unchanged to H9 under ADR 0006, and safe proof is not a substitute. |
