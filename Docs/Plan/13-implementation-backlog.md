...
| LM-025 | Implement downscale and ≤30-second, single-epoch, fixed-dimension HEIC-keyframe `MediaWriter`, canonical manifest, exact frame locators, per-frame/chunk hashes, and atomic directory finalization. Remove the AVAssetWriter/VideoToolbox encoder from the shipping path. | LM-018, LM-024 | Every chunk has one target/epoch/dimension; logical duration/frame ordering, exact-frame lookup, integrity, retained-only replacement, retention removal, and termination fixtures pass without executing a hardware video encoder. |
...
| LM-028 | Run eight-hour office soak plus accelerated focus/filter/fault/pixel-contamination suite. Measure the HEIC corpus, exact-frame decode, and projected 30-day retention; fix leaks, unbounded queues, corrupt manifests/assets, resolver errors, and recovery failures. | LM-027 | Resource/storage/decode budgets pass or a measured nonprivacy tuning ADR is adopted; automated scans find zero background/adjacent/excluded/stale sentinel in every canonical/derived location; `Results/LM-028/soak-report.md`. |
...
| LM-043 | Implement detail canvas and actor-owned exact-source-HEIC decode/cache with zoom/pan, frame stepping, corrupt-media recovery, and export-moment action. | LM-025, LM-041 | Decode latency gates pass; rapid selection cancellation shows the exact requested frame; corrupt file or manifest mismatch yields recovery UI. |
...
| LM-060 | Complete transactional retained-frame HEIC chunk replacement for moment/range deletion, vector compaction, thumbnail/text removal, tombstone recovery, and old-directory disposal. Never re-encode retained source frames. | LM-047, LM-051, LM-059 | Every delete crash point resumes/rolls back; only nondeleted frame assets remain integrity-valid, decodable, and searchable; deleted sentinels are absent from manifests and files. |
| LM-061 | Build forensic deletion verifier scanning DB/WAL/HEIC manifests and frame assets/thumbnails/vectors/logs/exports/helper projections for seeded sentinels. | LM-060 | All deletion fixture sentinels and frame IDs are absent after checkpoint/vacuum and old-directory disposal; signed report `Results/LM-061/deletion.json`. |
...
| LM-082 | Run 72-hour accelerated HEIC capture/enrichment/search/retention/delete fault soak with clock advancement and disk pressure. | LM-081 | No unbounded growth, corrupt searchable manifest/asset state, stuck leases, exclusion leak, orphan staging/ready directory, deleted sentinel, or unrecoverable restart. |
