...
| LM-005 | Run S1 foreground-window resolver/filter-epoch/dedup/HEVC spike and record production constants. | LM-002, LM-003; H2 if not already satisfied | Resolver, stale-frame, pixel-contamination, media, and resource gates pass or the listed nonprivacy fallback is adopted through ADR; `Decisions/0001-capture-media.md`. |
...
| LM-025 | Implement downscale and ≤30-second, single-epoch, fixed-dimension variable-frame-rate hardware HEVC `MediaWriter`, frame locators, hash, and finalization. | LM-018, LM-024 | Every chunk has one target/epoch/dimension; duration/PTS/frame counts, random decode, and termination fixtures pass. |
| LM-026 | Implement atomic coordinator among epoch/policy identity, media readiness, frame rows, and retryable jobs; no row points to partial or mismatched media. | LM-017, LM-022, LM-025 | Injected crashes/focus races restore invariants and never expose stale/mismatched targets; `Results/LM-026/state-machine.json`. |
| LM-027 | Connect visible pause/resume, foreground-window-only status, launch-at-login, idle, sleep/wake, target gaps, permission loss, low-disk stop, and safe restart. | LM-009, LM-023, LM-026; H3 only when launch-at-login confirmation appears | UI agrees within 250 ms; no hidden capture; every unavailable target/stop reason creates its exact typed gap. |
| LM-028 | Run eight-hour office soak plus accelerated focus/filter/fault/pixel-contamination suite. Fix leaks, unbounded queues, corrupt chunks, resolver errors, and recovery failures. | LM-027 | Budgets pass; automated scans find zero background/adjacent/excluded/stale sentinel in every canonical/derived location; `Results/LM-028/soak-report.md`. |
...
| LM-033 | Generate hash-verified 480-pixel HEIC searchable thumbnails asynchronously and recover/rebuild missing thumbnails. | LM-026 | Aspect/orientation/color fixtures pass; deletion and missing-file rebuild tests pass. |
...
| LM-035 | Implement merged-text records and external-content FTS5 maintenance for text, title, app, host/path, and future transcript source. | LM-032, LM-034 | FTS row count/integrity matches ready frames after insert/update/delete/rebuild. |
...
| LM-037 | Implement lexical `SearchEngine`: policy filters, BM25, deterministic boosts, evidence spans, stable keyset cursor, cancellation, and pagination. | LM-035, LM-036 | Ordering/cursor/property tests pass; prohibited frames never enter candidates; CLI-independent API. |
...
| LM-043 | Implement detail canvas and actor-owned HEVC random-access decode/cache with zoom/pan, frame stepping, corrupt-media recovery, and export-moment action. | LM-025, LM-041 | Decode latency gates pass; rapid selection cancellation shows correct frame; corrupt file yields recovery UI. |
...
| LM-058 | Implement exact 30-day retention and 20-GB cap worker, oldest-ready-chunk ordering, low-disk emergency stop, progress, and deterministic fake-clock tests. | LM-026, LM-047 | Bound never knowingly exceeds cap after cleanup; active chunk protected; derived rows/files cascade. |
...
| LM-060 | Complete transactional media-chunk rewrite for moment/range deletion, vector compaction, thumbnail/text removal, tombstone recovery, and old-file disposal. | LM-047, LM-051, LM-059 | Every delete crash point resumes/rolls back; only nondeleted frames remain decodable and searchable. |
| LM-061 | Build forensic deletion verifier scanning DB/WAL/media decode/thumbnails/vectors/logs/exports/helper projections for seeded sentinels. | LM-060 | All deletion fixture sentinels absent after checkpoint/vacuum; signed report `Results/LM-061/deletion.json`. |
...
| LM-077 | Add timestamped transcript spans to FTS/evidence/timeline with audio-source labels and no silent semantic summary. | LM-035, LM-044, LM-076 | Transcript search locates labeled fixtures within timestamp tolerance; evidence distinguishes transcription. |
...
| LM-081 | Complete five real workdays of personal dogfood, logging search failures, false exclusions, resource discomfort, crashes, and UX friction; fix release blockers. | LM-080; H5 | Five-day report, resource percentiles, issue dispositions, and zero known privacy/deletion blocker. |
| LM-082 | Run 72-hour accelerated capture/enrichment/search/delete fault soak with clock advancement and disk pressure. | LM-081 | No unbounded growth, corrupt searchable state, stuck leases, exclusion leak, or unrecoverable restart. |
